def compute_prefix_function(pattern: str) -> list:
    ...
    for current_position in range(1, pattern_length):
        while (
            matched_length > 0 and pattern[matched_length] != pattern[current_position]
        ):
...
def kmp_search(text: str, pattern: str) -> int:
    ...
    Обрати внимание: теперь аргументы (text, pattern).
    """
    pattern_length: int = len(pattern)
    ...
    prefix_table: list[int] = compute_prefix_function(pattern)
    matched_length: int = 0
